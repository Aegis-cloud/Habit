...
$cid=$_POST['sid'];
 include('connection.php');
   $sql_department = "SELECT * FROM sub_category where sub_id=$cid";
   $department_data = mysqli_query($conn,$sql_department);
   while($row = mysqli_fetch_assoc($department_data) ){
      $departname = $row['sub_name'];
      $departsid=$row['cat_id']
	  
?>
<form method="post" action="supdatenew.php"><label>Sid</label>
<input type="text" value="<?php echo $row['sub_id']; ?>" name="sid">
	<label>Sname</label>
<input type="text" name="cname" value="<?php echo $row['sub_name']; ?>">
	<label>Cid</label>
<input type="text" name="text1" value="<?php echo $row['cat_id']; ?>">
