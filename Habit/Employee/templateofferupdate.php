					...
					<div class="col-lg-12">
						<div class="banner-content">
						  <p class="text-white">&nbsp;</p>
                            <div>
								<div>Offer id </div>
    <form method="post" action="offerview.php">   
<select id="seldepart" name="seldepart" onchange='updateMyText()'>
   <option value="0">- Select -</option>
   <?php 
   // Fetch Department
   $con=new MySQLi("localhost","root","","habit");
   $sql_department = "SELECT * FROM offers";
   $department_data = mysqli_query($con,$sql_department);
   while($row = mysqli_fetch_assoc($department_data) ){
      $departid = $row['offerid'];
      $depart_name = $row['offeritem'];
	   $depart_sid=$row['offerdiscount'];
	   $depart_price=$row['offerpercent'];
	   $depart_period=$row['id'];
