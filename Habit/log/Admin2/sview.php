...
$cid=$_POST['sid'];
 $con=new MySQLi("localhost","root","","tms");
   $sql_department = "SELECT * FROM subcategory where sid=$cid";
   $department_data = mysqli_query($con,$sql_department);
   while($row = mysqli_fetch_assoc($department_data) ){
      $departname = $row['sname'];
      $departsid=$row['cid']
	  
?>
<form method="post" action="supdatenew.php"><label>Sid</label>
<input type="text" value="<?php echo $row['sid']; ?>" name="sid">
	<label>Sname</label>
<input type="text" name="cname" value="<?php echo $row['sname']; ?>">
	<label>Cid</label>
<input type="text" name="text1" value="<?php echo $row['cid']; ?>">
